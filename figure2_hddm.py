...
# more handy imports
from utils_plot import results_long2wide_hddmnn, seaborn_style, corrfunc
import corrstats
seaborn_style()
...
sns.barplot(x="model", y="dic", ax=ax[0], data=mdcomp, bottom=mdcomp.loc[mdcomp.model == 'ddm_nohist', 'dic'], palette=pal3)
ax[0].set(ylabel=r'$\Delta$DIC', xlabel='', xticklabels=['DDM', 'angle', 'weibull'])
sns.barplot(x="model", y="aic", ax=ax[1], data=mdcomp, bottom=mdcomp.loc[mdcomp.model == 'ddm_nohist', 'aic'], palette=pal3)
ax[1].set(ylabel=r'$\Delta$AIC', xlabel='', xticklabels=['DDM', 'angle', 'weibull'])
sns.barplot(x="model", y="bic", ax=ax[2], data=mdcomp, bottom=mdcomp.loc[mdcomp.model == 'ddm_nohist', 'bic'], palette=pal3)
ax[2].set(ylabel=r'$\Delta$BIC', xlabel='', xticklabels=['DDM', 'angle', 'weibull'])
...
models = ['ddm_nohist',  'ddm_prevresp_z',
          'ddm_prevresp_dc',
          'ddm_prevresp_dcz']
mdcomp = pd.DataFrame()
for mod in models:
...
for ax in fig.axes:
    ...
    plt.xticks(rotation=40, ha='right')
fig.tight_layout()
fig.savefig(os.path.join(figpath, 'modelcomp_ddms.png'))
...
md_wide = results_long2wide_hddmnn(pd.read_csv(os.path.join(modelpath, 'ddm_prevresp_dcz', 'results_combined.csv')))
...
sns.despine(trim=True)
plt.tight_layout()
fig.savefig(os.path.join(figpath, 'scatterplot_trainingchoiceworld_prevchoice_dcz.png'))
